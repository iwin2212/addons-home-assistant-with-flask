...
class XiaomiCloudConnector:
    ...
    def login_step_1(self):
        ...
        valid = response.status_code == 200 and "_sign" in self.to_json(
            response.text)
        if valid:
    ...
    def generate_signature(url, signed_nonce, nonce, params):
        ...
        signature = hmac.new(base64.b64decode(
            signed_nonce), msg=signature_string.encode(), digestmod=hashlib.sha256)
        ...
        for current_server in servers:
            ...
            if devices is not None:
                ...
                for device in devices["result"]["list"]:
                    dev_data = {}
                    dev_data['server'] = current_server
                    if "name" in device:
                        dev_data['name'] = device["name"]
                    if "did" in device:
                        dev_data['id'] = device["did"]
                    ...
                    if "localip" in device:
                        dev_data['ip'] = device["localip"]
                    if "token" in device:
                        dev_data['token'] = device["token"]
                    if "model" in device:
